...
void processor(const std::string inputFile, const std::string outputFile, bool& done, uint64_t& processed, uint64_t& found, int waterfallX)
{
    ...
    while (input.good()) {
        ...
        int usedTrees = 0;
        if (generator::ChunkGenerator::populate(seed, waterfallX, &usedTrees)) {
...
{
    int usedTrees = 0;
    bool val = generator::ChunkGenerator::populate(122221704418878ULL ^ 0x5DEECE66DULL, 12, &usedTrees);
    std::cout << val << " " << usedTrees << std::endl;
    ...
    for (int i = 0; i < threadCount; ++i) {
        std::string fileName = "/media/balintcsala/Main/out_blob" + std::to_string(i + first) + ".txt";
